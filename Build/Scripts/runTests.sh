...
read -r -d '' HELP <<EOF
t3oidc test runner. Execute unit test suite and some other details.
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/t3oidc/${1}"
else
    case ${TEST_SUITE} in
        acceptance)
            TEST_FILE="Web/typo3conf/ext/t3oidc/Tests/Acceptance"
            ;;
        functional)
            TEST_FILE="Web/typo3conf/ext/t3oidc/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/t3oidc/Tests/Unit"
            ;;
    esac
fi
